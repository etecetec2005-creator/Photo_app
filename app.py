...
st.set_page_config(page_title="写真解析", layout="centered")import streamlit as st
import streamlit.components.v1 as components
import requests
import base64
from PIL import Image
...
API_URL = "https://api.japan-ai.co.jp/chat/v2"
API_MODEL_TITLE = "gpt-4o-mini"
# データセットID（もし送付画像のみで分析する場合はartifactIdsを空にするか、プロンプトで制御します）
ARTIFACT_ID = "ba011f66-7e30-49ee-b7a0-2417a7ee26bf"
...
if img_file:
    ...
    with st.spinner("JAPAN AI が画像からタイトルを生成中..."):
        try:
            ...
            # APIリクエストの作成
            # プロンプトを「送付された画像を直接見る」ように変更
            payload = {
                "model": API_MODEL_TITLE,
                "prompt": "添付された画像を分析し、その内容を一言で表す短いタイトル（20文字以内）を【日本語】で作成してください。余計な説明や挨拶は一切不要です。タイトルのみを出力してください。",
                "stream": False,
                "temperature": 0.0,
                # artifactIdsが「見つかりません」の原因になる場合は、一時的にリストを空にするか、
                # プロンプトで「添付画像」を強調します。
