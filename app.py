...
if img_file:
    ...
    # 2. 住所特定プロセス
    # URLにまだ住所がない（addrパラメータが空）場合のみ、JSを動かす
    if not addr_from_url:
        st.subheader("📍 撮影場所を特定中...")
        
        # 取得完了後に一度だけリロードさせるためのスクリプト
        address_script = """
            ...
            async (pos) => {
                try {
                    const response = await fetch(`https://nominatim.openstreetmap.org/reverse?format=json&lat=${pos.coords.latitude}&lon=${pos.coords.longitude}`, {
                        headers: { 'Accept-Language': 'ja', 'User-Agent': 'StreamlitPhotoApp/2.0' }
                    });
                    ...
                    // 重要：現在のURLにaddrが含まれていない場合のみ遷移を実行（無限ループ防止）
        ...
        st.info("住所の特定を待っています...（特定されると自動的に解析が始まります）")
